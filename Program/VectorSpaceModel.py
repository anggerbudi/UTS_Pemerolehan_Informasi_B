...
class Program:
    ...
    def process_document(self, doc):
        with open(doc, 'r') as file:
            text = self.remove_special_char(file.read())
    ...
    def text_loader_and_words_collector(self):
        ...
        for idx, doc in enumerate(glob.glob(self.file_folder), start=1):
            words = self.process_document(doc)
            word_freq = self.find_freq_and_unique_word(words)
            files_with_index[idx] = os.path.basename(doc)
            unique_words_all.update(word_freq.keys())
            print('ini dict global : ', word_freq)
    ...
    def calculate_tfidf_for_words(self, unique_words_all, linked_list_data, files_with_index):
    ...
    def vec_d_matrix(self, unique_words_all, files_with_index, linked_list_data):
    ...
    def query_input(self, total_vocab, dict_words, linked_list_data):
        ...
        wordsq = [self.stemmer.stem(word.lower()) for word in word_tokenize(query)
                  if len(word) >= 1 and word.lower() not in self.stop_words]
        dict_query = self.find_freq_and_unique_word(wordsq)
